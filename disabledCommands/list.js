...
const restraints = [
	...
	//["Chastity Bras", chastitybratypes.sort((a, b) => a.name.localeCompare(b.name)).map((heavy) => ({ name: heavy.name, value: `Denial coefficient: ${heavy.denialCoefficient}`, inline: false }))],
	...
	["Collars", process.collartypes.sort((a, b) => a.name.localeCompare(b.name)).map((heavy) => ({ name: heavy.name, value: "-# *No description*", inline: false }))],
...
function buildMessage(type, page, details) {
	...
	return {
		...
		embeds: [
			details
				? { title: typeArr[0], fields: typeArr[1].slice(start, start + PAGE_SIZE) }
				: {
						title: typeArr[0],
		...
		components: [
			{ type: ComponentType.ActionRow, components: [{ type: ComponentType.StringSelect, custom_id: `list-select-${page}-${+details}`, options: restraintOptions, placeholder: "Change restraint type..." }] },
			{
				type: ComponentType.ActionRow,
				components: [
					{ type: ComponentType.Button, custom_id: `list-${type}-${page - 1}-${+details}`, label: "← Prev", disabled: page == 0, style: ButtonStyle.Secondary },
					{ type: ComponentType.Button, custom_id: `list-${type}-${page}-${+details}`, label: `Page ${page + 1} of ${maxPage + 1}`, disabled: true, style: ButtonStyle.Secondary },
					{ type: ComponentType.Button, custom_id: `list-${type}-${page + 1}-${+details}`, label: "Next →", disabled: page == maxPage, style: ButtonStyle.Secondary },
					{ type: ComponentType.Button, custom_id: `list-${type}-${page}-${+!details}`, label: details ? "Hide details" : "Show details", style: ButtonStyle.Secondary },
				],
			},
		],
	};
