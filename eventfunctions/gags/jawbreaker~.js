...
async function tick(serverID, userID, data) {
    ...
    if (getUserVar(serverID, userID, "confectionaryDissolveTimer") < Date.now() && getGag(serverID, userID, "jawbreaker~") && process.recentmessages[serverID][userID]) {
        if(getGag(serverID, userID, "jawbreaker~").intensity > 1){
            ...
            messageSendChannel(`<@${userID}>'s licking has shrunk ${getPronouns(serverID, userID, "possessiveDeterminer")} Jawbreaker Gag a little bit! The sacchrine flavors remind ${getPronouns(serverID, userID, "object")} of wonderful things!`, process.recentmessages[serverID][userID])
        }
        else {
            ...
            messageSendChannel(`<@${userID}>'s Jawbreaker Gag dissolves away, but the sweet juices of the candy linger on ${getPronouns(serverID, userID, "possessiveDeterminer")} tongue, along with a desire to share them with someone!`, process.recentmessages[serverID][userID])
        }
    }
}
