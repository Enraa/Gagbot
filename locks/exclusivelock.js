...
/***********
 * This is a small lock that prevents the wearer from manipulating the restraint. Everyone else may freely manipulate it. 
 ***********/
...
exports.lockinteraction = function (interaction, data, update = false) {
    ...
    // Main Title text
    let maintitle = new TextDisplayBuilder().setContent(`## Applying an Exclusive Lock to ${(getLockAwaiting(data.uuid)?.userID == interaction.user.id) ? "your" : `<@${getLockAwaiting(data.uuid)?.userID}>'s`} ${getLockAwaiting(data.uuid)?.restraintname}`);
    ...
    if (command == "setkeyholder") {
        ...
        try {
            ...
            if (appliedlock == "NoAccess") {
                interaction.update({ components: [new TextDisplayBuilder().setContent(`You don't have access to apply an Exclusive Lock to <@${userID}>'s ${lockrestraint}.`)] })
            }
                ...
                await handleApplyLock(interaction.guildId, interaction.user, await interaction.guild.members.fetch(userID), uuid).then(
                    async (success) => {
                        ...
                        sendLockToast({ serverID: interaction.guildId, userID: userID, actionuser: interaction.user.id, actiontype: "lock", locktype: "exclusivelock", restraintname: lockrestraint, restrainttype: lockrestrainttype, targettype: targettype })
                    },
                    async (reject) => {
...
exports.lockStatus = function (data) {
    ...
    return `${lockemoji} Locked from ${getPronouns(lock.serverID, lock.userID, "reflexive")} but others can touch!`
}
