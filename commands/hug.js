...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("hug")
		.setDescription("Attempt to hug someone")
        .addUserOption((opt) => opt.setName("user").setDescription("Who to hug?")),
	async execute(interaction) {
		try {
            ...
            await handleTouchEvent(interaction.guildId, interaction.user, targetuser, "hug").then(
                async (success) => {
                ...
                async (reject) => {
                    let nomessage = `${targetuser} rejected the hug!`;
                    if (reject == "Error") {
